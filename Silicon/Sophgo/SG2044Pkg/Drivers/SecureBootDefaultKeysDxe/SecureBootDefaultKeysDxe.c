/** @file
  This driver init default Secure Boot variables infclude
  the information for verifying PK.
  ...
  )
{
  ...
  Status = SecureBootInitRootKey ();
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Cannot initialize RK: %r\n", __func__, Status));
    return Status;
  }

  Status = SecureBootInitPKSignature ();
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Cannot initialize PK signature: %r\n", __func__, Status));
    return Status;
  }
