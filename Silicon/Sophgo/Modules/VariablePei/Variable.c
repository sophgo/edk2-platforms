  ...
  )
{
  ...
  GuidHob = GetFirstGuidHob (&gEfiSophgoPeiVariableGuid);
  if (GuidHob == NULL) {
    DEBUG ((DEBUG_INFO, "%a: Pei variable flash info not found\n", __func__));
...
EFI_STATUS
EFIAPI
PeiGetVariableFlashNvStorageInfo (
  ...
  )
{
  ...
  switch (Type) {
    ...
    case VariableStoreTypeNv:
        ...
        Status = PeiGetVariableFlashNvStorageInfo (&NvStorageBase, &NvStorageSize64);
  ...
  )
{
  ...
  for ( PtrTrack->CurrPtr = GetStartPointer (VariableStoreHeader)
      ; IsValidVariableHeader (PtrTrack->CurrPtr)
      ; PtrTrack->CurrPtr = GetNextVariablePtr (StoreInfo, PtrTrack->CurrPtr, PtrTrack->CurrPtr)
      )
  {
    if (VariableName[0] == 0) {
      ...
      if (CompareGuid (VendorGuid, GetVendorGuidPtr (PtrTrack->CurrPtr, StoreInfo->AuthFlag))) {
        Point = (VOID *)GetVariableNamePtr (PtrTrack->CurrPtr, StoreInfo->AuthFlag);

        ASSERT (NameSizeOfVariable (PtrTrack->CurrPtr, StoreInfo->AuthFlag) != 0);
        if (CompareMem (VariableName, Point, NameSizeOfVariable (PtrTrack->CurrPtr, StoreInfo->AuthFlag)) == 0) {
          if (PtrTrack->CurrPtr->State == VAR_ADDED || PtrTrack->CurrPtr->State == (VAR_IN_DELETED_TRANSITION & VAR_ADDED)) {
            InDeletedVariable = PtrTrack->CurrPtr;
          }
        }
      }
