/** @file
  ...
  Copyright (C) 2025, SOPHGO Technologies Inc., Ltd. All rights reserved.<BR>
...
Scope(_SB)
{
  ...
  Device (GED0) {
    ...
    // SWPORTA_DDR
    OperationRegion(PDDR, SystemMemory, 0x7030009004, 4)
    Field(PDDR, DWordAcc, NoLock, Preserve) {
      SWPO, 32
    }

    // INTEN
    OperationRegion(INTE, SystemMemory, 0x7030009030, 4)
    Field(INTE, DWordAcc, NoLock, Preserve) {
      INTN, 32
    }

    // INTMASK
    OperationRegion(INTM, SystemMemory, 0x7030009034, 4)
    Field(INTM, DWordAcc, NoLock, Preserve) {
      MASK, 32
    }

    // INTTYPE_LEVEL
    OperationRegion(INTL, SystemMemory, 0x7030009038, 4)
    Field(INTL, DWordAcc, NoLock, Preserve) {
      LEVL, 32
    }

    // INT_POLARITY
    OperationRegion(INTP, SystemMemory, 0x703000903C, 4)
    Field(INTP, DWordAcc, NoLock, Preserve) {
      POLA, 32
    }

    // INTSTATUS
    OperationRegion(INTS, SystemMemory, 0x7030009040, 4)
    Field(INTS, DWordAcc, NoLock, Preserve) {
      STAS, 32
    }

    // RAW_INTSTATUS
    OperationRegion(INTR, SystemMemory, 0x7030009044, 4)
    Field(INTR, DWordAcc, NoLock, Preserve) {
      RAWS, 32
    }

    // PORTA_EOI
    OperationRegion(PEOI, SystemMemory, 0x703000904C, 4)
