...
SMBIOS_PLATFORM_DXE_TABLE_FUNCTION (PlatformBios) {
  ...
  while (InputData->Hdr.Type != NULL_TERMINATED_TYPE) {
    ...
    if (SearchValueBySectionAndName ("BIOS Information", "vendor", value) == 0) {
    ...
    if (SearchValueBySectionAndName ("BIOS Information", "version", value) == 0) {
	    AsciiStrToUnicodeStrS (value, UnicodeStr, SMBIOS_UNICODE_STRING_MAX_LENGTH);
	    HiiSetString (mSmbiosPlatformDxeHiiHandle, InputStrToken->TokenArray[1], UnicodeStr, NULL);
    }

    if (SearchValueBySectionAndName ("Date Time", "date", value) == 0) {
      AsciiStrToUnicodeStrS (value, UnicodeStr, SMBIOS_UNICODE_STRING_MAX_LENGTH);

      if (SearchValueBySectionAndName ("Date Time", "time", value) == 0) {
