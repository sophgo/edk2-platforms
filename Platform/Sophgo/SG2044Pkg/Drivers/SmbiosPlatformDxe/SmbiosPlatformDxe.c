...
#include "../../Include/Library/IniParserLib.h"
...
#define CONFIG_INIT_LOAD_ADDR	0x89000000
...
INT32
SearchValueBySectionAndName (
    CONST CHAR8 *Section,
    CONST CHAR8 *Name,
    CHAR8 *Value
) {
    for (UINTN i = 0; i < EntryCount; i++) {
      //DEBUG((DEBUG_INFO, "Section =%a name=%a linpeng %a %a\n", Section, Name, gIniEntries[i].Section, gIniEntries[i].Name));
...
IniHandler (
    ...
    //DEBUG((DEBUG_INFO, "Handler called with Section: %a, Name: %a, Value: %a\n", Section, Name, Value));
...
{
    CONST CHAR8 *Buffer = (CHAR8 *)CONFIG_INIT_LOAD_ADDR;

    INT32 result = IniParseString(Buffer, Handler, User);
    DEBUG((DEBUG_INFO, "IniParseString returned: %d\n", result));
  ...
  )
{
  ...
  Status = ConfIniParse (IniHandler, NULL);
  if (EFI_ERROR(Status)) {
    DEBUG ((DEBUG_ERROR, "Could not install SMBIOS Table Type. %r\n", Status));
  }
