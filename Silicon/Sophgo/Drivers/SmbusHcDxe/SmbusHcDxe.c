...
#define AST2600_SSIF_SMBUS_ADDR 0x10
...
# else
      // DEBUG ((DEBUG_INFO, "#########-SmbusHcCommonExecute-Write-########.\n"));
      DataLen = *Length;
      CopyMem (&WriteTemp[0], Buffer, *Length);
      DEBUG ((DEBUG_INFO, "W %d: ", DataLen));
      DEBUG ((DEBUG_INFO, "Addr 0x%x: ", SlaveAddress.SmbusDeviceAddress));
      for (Idx = 0; Idx < DataLen; Idx++) {
        DEBUG ((DEBUG_INFO, "0x%x ", WriteTemp[Idx]));
      }
      DEBUG ((DEBUG_INFO, "\n"));
      Status = mI2cMasterProtocol->Write (mI2cMasterProtocol,
                                      mI2cBusNum,
                                      /* SlaveAddress.SmbusDeviceAddress */AST2600_SSIF_SMBUS_ADDR,
...
# else
      // DEBUG ((DEBUG_INFO, "#########-SmbusHcCommonExecute-read-########.\n"));
      WriteTemp[0] = Command;
      DataLen = *Length;
...
# endif
      ...
      DEBUG ((DEBUG_INFO, "R %d: ", DataLen));
      for (Idx = 0; Idx < DataLen; Idx++) {
        DEBUG ((DEBUG_INFO, "0x%x ", ReadTemp[Idx]));
      }
      DEBUG ((DEBUG_INFO, "\n"));
      ...
      //
      if (PecCheck) {
        CrcTemp[0] = I2C_WRITE_ADDRESS (/* SlaveAddress.SmbusDeviceAddress */AST2600_SSIF_SMBUS_ADDR);
        CrcTemp[1] = Command;
        CrcTemp[2] = I2C_READ_ADDRESS (/* SlaveAddress.SmbusDeviceAddress */AST2600_SSIF_SMBUS_ADDR);
  ...
  )
{
  ...
  mI2cBusNum = FixedPcdGet32 (PcdSsifI2cBusNum);
