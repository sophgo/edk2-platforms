...
GetTimeData (
  VOID
    ...
    UnicodeSPrint(
        gFrontPagePrivate.TimeData.Time,
        sizeof(gFrontPagePrivate.TimeData.Time),
        L"%02d:%02d:%02d",
        CurrentTime.Hour,
        CurrentTime.Minute,
        CurrentTime.Second
    ...
    Status = gRT->SetVariable(
        ...
        EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS, 
        sizeof(TIME_DATA),                   
        &gFrontPagePrivate.TimeData         
    );
    Status = HiiSetBrowserData (
       &mFrontPageGuid,                    
       VariableName,                      
       sizeof(TIME_DATA),                   
       (UINT8 *)&gFrontPagePrivate.TimeData,       
       NULL
    );
...
{   
    ...
    EFI_QUESTION_ID  QuestionId = 0x1234;
    EFI_VARSTORE_ID  VarStoreId = VAR_DYNAMIC_TIME_VARID; 
    UINT16           VarOffset = OFFSET_OF(TIME_DATA, Time);
    EFI_STRING_ID    Prompt = STRING_TOKEN(STR_TIME_PROMPT);
    EFI_STRING_ID    Help = STRING_TOKEN(STR_TIME_HELP);
    UINT8            QuestionFlags = EFI_IFR_FLAG_READ_ONLY;
    UINT8            MinSize = 8;                    
    UINT8            MaxSize = 8;                     
    ...
    UINT8 *OpCode = HiiCreateStringOpCode(
                    StartOpCodeHandle,
                    QuestionId,
                    VarStoreId,
                    VarOffset,
