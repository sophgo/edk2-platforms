...
#include <Library/SecureBootVariableVerifyPK.h>
...
/**
  Check if PK has passed validation with the RK and PK signature

  @return EFI_SUCCESS           PK verification successful
  @return Others                PK verification failed
  ...
  )
{
  ...
  UINT8                   *PKSig;
  UINTN                   PKSigSize;
  EFI_SIGNATURE_LIST      *PKCertList;
  EFI_SIGNATURE_DATA      *PKCert;
  UINT8                   *PKCertData;
  UINTN                   PKCertSize;
  EFI_SIGNATURE_LIST      *RKCertList;
  EFI_SIGNATURE_DATA      *RKCert;
  UINT8                   *RKCertData;
  UINTN                   RKCertSize;
  VOID                    *RsaContext;
  ...
  if (EFI_ERROR (Status) || (PKSize == 0) || (PK == NULL)) {
    DEBUG ((DEBUG_INFO, "Variable %s not found, skip the PK verification process\n", EFI_PLATFORM_KEY_NAME));
    return EFI_NOT_FOUND;
  }
  PKCertList   = (EFI_SIGNATURE_LIST *)PK;
  PKCert       = (EFI_SIGNATURE_DATA *)((UINT8 *)PKCertList + sizeof (EFI_SIGNATURE_LIST) + PKCertList->SignatureHeaderSize);
  PKCertData   = PKCert->SignatureData;
  PKCertSize   = PKCertList->SignatureSize - (sizeof (EFI_SIGNATURE_DATA) - 1);
  //
  //Check if the RK and PK signature exits
  //
  Status = AuthServiceInternalFindVariable (
          EFI_RK_VARIABLE_NAME,
          &gEfiGlobalVariableGuid,
          (VOID **)&RK,
          &RKSize
  );
  if (EFI_ERROR (Status) || (RKSize == 0) || (RK ==NULL)) {
    DEBUG ((DEBUG_INFO, "Variable %s not found, skip the PK verification process\n", EFI_RK_VARIABLE_NAME));
    return EFI_NOT_FOUND;
  }
  RKCertList   = (EFI_SIGNATURE_LIST *)RK;
  RKCert       = (EFI_SIGNATURE_DATA *)((UINT8 *)RKCertList + sizeof (EFI_SIGNATURE_LIST) + RKCertList->SignatureHeaderSize);
  RKCertData   = RKCert->SignatureData;
  RKCertSize   = RKCertList->SignatureSize - (sizeof (EFI_SIGNATURE_DATA) - 1);
  Status = AuthServiceInternalFindVariable (
          EFI_PK_SIG_VARIABLE_NAME,
          &gEfiGlobalVariableGuid,
          (VOID **)&PKSig,
          &PKSigSize
  );
  if (EFI_ERROR (Status) || (PKSigSize == 0) || (PKSig == NULL)) {
    DEBUG ((DEBUG_INFO, "Variable %s not found, skip the PK verification process\n", EFI_PK_SIG_VARIABLE_NAME));
    return EFI_NOT_FOUND;
  }
  //
  //verify only exits with both PK and PK signature
  //
  if ( ((RKSize != 0) && (RK != NULL)) &&
      ((PKSigSize != 0) && (PKSig != NULL)) )
  {
    Sha256HashAll (PKCertData, PKCertSize, MessageHash);
    RsaContext = RsaNew ();
    RsaGetPublicKeyFromX509 (RKCertData, RKCertSize, &RsaContext);
    if (RsaPkcs1Verify (RsaContext,
          MessageHash,
          SHA256_DIGEST_SIZE,
          PKSig,
          PKSigSize)) {
      DEBUG ((DEBUG_INFO, "PK verified successfully\n"));
      Status = EFI_SUCCESS;
    } else {
      DEBUG ((DEBUG_INFO, "PK verified failed\n"));
  ...
  )
{
  ...
  //
  // Check if the RK and PK signature exits, if not just pass.
  // PK need to be verified only in user mode 
  //
  if ( (mPlatformMode == USER_MODE) &&
        !(InCustomMode ()) )
  {
    Status = PKRsaPkcs1Verify ();
