...
/**
  ...
  @param[in]  Str2    The second string, can be NULL.
  ...
  )
{
  ...
  if (Str2 == NULL) {
    ...
    StrCount = 2;
    MaxStrLen = MAX (StrLen (Str1), StrLen (Str2));
  }
  ...
  ClearPopUp (EFI_BACKGROUND_LIGHTGRAY, MaxStrLen, StrCount);
  ...
  )
{
  ...
  CHAR16           *PressEnterString;
  EFI_STRING_ID    PressEnterToken;

  PressEnterToken = STRING_TOKEN (STR_PRESS_ENTER_TO_REBOOT);
  PressEnterString = HiiGetString (gFirmwareUpdateHandle, PressEnterToken, NULL);
  UpdateSuccString = HiiGetString (gFirmwareUpdateHandle, TokenToUpdate, NULL);

  PopupInformation (UpdateSuccString, PressEnterString);
  ...
  )
{
  ...
  PressEnterToken = STRING_TOKEN (STR_PRESS_ENTER_TO_CONTINUE);
  PressEnterString = HiiGetString (gFirmwareUpdateHandle, PressEnterToken, NULL);
  UpdateSuccString = HiiGetString (gFirmwareUpdateHandle, TokenToUpdate, NULL);
  ...
  )
{
  ...
  if (PromptSkipVariable) {
    ...
    //
    // Only when SecureBoot is not enabled, is it allowed
    // to change the variable area.
    //
    if (IsSecureBootEnable == 0) {
      ...
      ClearPopUp (EFI_BACKGROUND_LIGHTGRAY, StrLen (PromptString), 2);
    } else {
      PressEnterToContinue (STRING_TOKEN (STR_VARIABLE_LOCKED));
      SkipVariable = TRUE;
    }
  }
...
)
{
  ...
  if (QuestionId == UPDATE_FIRMWARE_KEY) {
    Status = FirmwareVerify (FileBuffer, FileSize, Nor, NorFlashProtocol);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "Firmware verification failed!\n"));
      PressEnterToContinue (STRING_TOKEN (STR_FIRMWARE_VERIFY_FAIL));
      FreePool (FileBuffer);
      return FALSE;
    } else {
      DEBUG ((DEBUG_ERROR, "Firmware verification passed!\n"));
      PressEnterToContinue (STRING_TOKEN (STR_FIRMWARE_VERIFY_PASS));
    }
  }
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (EFI_ERROR(Status)) {
      ...
      PressEnterToReset (TokenToUpdate2);
    }
  }
