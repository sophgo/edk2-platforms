  ...
  )
{
  ...
  *BufferPtr++ = BASIC_MODE_STOP;
  ...
  RetryCount = 0;
  while (TRUE) {
    Status = SophgoSerialPortWrite (Request, RequestLength);
  ...
  )
{
  ...
  while (ReadBytes < IPMI_SERIAL_MAXIMUM_PACKET_SIZE_IN_BYTES) {
    ...
    while (SophgoSerialPortPoll ()) {
      SophgoSerialPortRead (&Character, sizeof (Character));
  ...
  DEBUG ((DEBUG_VERBOSE, "Ipmi serial RecvLen: 0x%x\n", BufferSize));
  for (Index = 0; Index < BufferSize; Index ++) {
    DEBUG ((DEBUG_VERBOSE, "0x%x ", Buffer[Index]));
