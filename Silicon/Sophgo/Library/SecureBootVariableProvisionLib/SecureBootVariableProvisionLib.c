/** @file
  ...
  keys and databases. And it provides functions to get/set 
  signatures and certificates for verifying the PK.
  ...
  )
{
  ...
  //
  //check if root key variable exits, if not don't set variable
  //
  Status = GetVariable2 (EFI_RK_VARIABLE_NAME, &gEfiGlobalVariableGuid, (VOID **)&Data, &DataSize);
  if (Status == EFI_SUCCESS) {
  ...
  // Variable does not exist, can be initialized
  //
  DEBUG ((DEBUG_INFO, "Variable %s does not exist, can be initialized.\n", EFI_RK_VARIABLE_NAME));

  Status = SecureBootFetchData (&gEfiRootKeyFileGuid, &SigListsSize, &EfiSig);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  KeyIndex = 0;
  //
  //check if PK key signature variable exits, if not don't set variable
  //
  Status = GetVariable2 (EFI_PK_SIG_VARIABLE_NAME, &gEfiGlobalVariableGuid, &Buffer, &Size);
  if (Status == EFI_SUCCESS) {
