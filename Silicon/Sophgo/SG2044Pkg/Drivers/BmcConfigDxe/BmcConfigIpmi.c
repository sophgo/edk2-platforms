  ...
  )
{
  ...
  if (AsciiStrLen ((const CHAR8 *)GetUserNameResponse.UserName) > 0) {
    ...
    DEBUG((DEBUG_INFO, "UserName:%a-%s,UserId: %d\n", (const CHAR8 *)GetUserNameResponse.UserName,UserName, UserId));
    return EFI_SUCCESS;
  } else {
 ...
 )
{
  ...
  DEBUG((DEBUG_INFO,"%a(),Len:%d, data %a , UserId %x over\n",__FUNCTION__,sizeof(IPMI_SET_USER_PASSWORD_CMD)+AsciiStrLen (UserPassword), SetPasswordRequest.PasswordData, SetPasswordRequest.UserId));
  Status = IpmiSubmitCommand (
            IPMI_NETFN_APP,
  ...
  )
{
     ...
     && (DeviceId.CompletionCode == IPMI_COMP_CODE_NORMAL))
  {
    ...
    DEBUG((DEBUG_INFO, "bmc fw version:%0x.%0x\n", DeviceId.FirmwareRev1.Bits.MajorFirmwareRev, DeviceId.MinorFirmwareRev));
    DEBUG((DEBUG_INFO, "bmc manufacturer id:%0x %0x %0x\n", DeviceId.ManufacturerId[0], DeviceId.ManufacturerId[1],DeviceId.ManufacturerId[2]));
    DEBUG((DEBUG_INFO, "ipmi version:%02x %02x\n", DeviceId.SpecificationVersion & 0x0F, (DeviceId.SpecificationVersion >> 4) & 0x0F));
  }
